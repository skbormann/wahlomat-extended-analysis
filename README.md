...
| Run analysis for an **individual** election | **Done (CLI, id-based)** — After **`build-csv`**, use **`python wahlomat.py graphs --list-elections`** (or [build_graphs_from_csv.py](build_graphs_from_csv.py) **`--list-elections`**) to list valid **`election_id`** values, then run **`--election ID`** (repeat or comma-separated) or **`ELECTION_IDS`**; optional **`--graph`** limits plot types (`c_matrix`, `pca_map`, `pca_influences`). Same flags work via **`python wahlomat.py graphs …`** (passthrough). Selection is intentionally by exact **`election_id`** (no interactive menu, no display-name lookup via **`election_metadata.csv`**). See [Analysis steps (detailed reference)](#analysis-steps-detailed-reference). |
| **Update** **`build_dataframe.py`** when new elections appear | **Not planned** — `build-csv` intentionally does a full rebuild for simplicity and correctness (on typical data it completes in ~tens of seconds). For workbook-only updates (the 2021+ distribution), use **`update-csv`** / **`refresh-excel`** instead. |
