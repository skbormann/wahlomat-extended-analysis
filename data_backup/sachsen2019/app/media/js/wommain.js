...
function bindTriggers() {
	...
	$('.wom_all_checkbox .mark-all-checkbox').on('click', function() {
	    allChecked = true;
    ...
    // Filter accordion
	var womAccordion = $('.wom-filter-accordion');
	var womAccordionBtn = $('.wom-filter-accordion .wom-filter-accordion-btn');
	var womAccordionPanel = $('.wom-filter-accordion-panel');
	...
	// Tabs component
	var womTabsContainer = $('.wom-tabs');
	var womTabsBtn = $('.wom-tabs .wom-tabs-btn');
	var womTabsPanel = $('.wom-tabs .wom-tabs-panel');
		...
		womTabsPanel.each(function(index, panel){
			...
			if (S_nDetailergebnisPartei > 0) {
				if (index === 1) {
					...
					$(window.location.hash).attr('aria-selected', 'true');
	...
	var womAnswerAccordions = $('.wom-answer-item');
	if (womAnswerAccordions) {
		womAnswerAccordions.each(function(){
			var button = $(this).find('.wom-answer-btn');
...
function slider(){
    ...
    pa_p.find('.wom_partei_balken').on('click',function() {
			...
			$(this).parent().toggleClass('wom_on');
	...
	$('.wom-thesis-item').find('.wom_antworten_box div').css('display', 'none');
	$('.wom-thesis-item').find('.wom_antworten_box.wom_on div').css('display', 'block');
...
function ergebnis_filter_gewichtung(gewichtungFilter) {
    if (gewichtungFilter == 1) {
        $('#wom ul.wom_ergebnis_list_gewichtet').css('display','block');
        $('#wom ul.wom_ergebnis_list_nichtgewichtet').css('display','none');
        S_nFilter_GewichtedAuswahl = 1;
    } else {
        $('#wom ul.wom_ergebnis_list_gewichtet').css('display','none');
        $('#wom ul.wom_ergebnis_list_nichtgewichtet').css('display','block');
