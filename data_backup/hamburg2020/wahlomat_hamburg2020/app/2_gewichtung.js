...
function ergebnis(zumErgebnis) {
    ...
    if (bGewichted) {
        S_nFilter_GewichtedFilter   = 1;
        S_nFilter_GewichtedAuswahl  = 1;
    } else {
        S_nFilter_GewichtedFilter   = 0;
        S_nFilter_GewichtedAuswahl  = 2;
    }
    if (S_nAuswertung == 1) {
        if (zumErgebnis == 4) {
            replaceIFrame(4);
        } else if (zumErgebnis == 5) {
            // Zum Detailergebnis
            replaceIFrame(5);
        } else {
            replaceIFrame(3);
...
function write_2_gewichtung(){
    ...
    rw += '<div class="wom_main_wrapper' + msl_get_view_class() + ' nobg" id="wom" lang="de">';
		        ...
		        rw += '<main class="wom_main_content">';
                    ...
                    rw += '<nav class="wom_breadcrumb" id="bnbreadcrumb">';
                        ...
                        rw += '<ul class="wom_breadcrumb_list">';
							rw += '<li class="wom_first_position wom_active" aria-current="page" title="' 
							...
							rw += '<li';
							    if (S_nAuswertung == 1) {
							        rw += ' class="wom_clickable" id="link_gw_parteiauswahl">';
							        rw += '<a href="" onclick="ergebnis(3);return false;">' 
							            + wom_html(WOMT_aTexte["bc_3_partei"][S_nSprache])
							        + '<\/a>';
							    } else {
							        rw += '><span>' + wom_html(WOMT_aTexte["bc_3_partei"][S_nSprache])+ '</span>';
							    }
							    rw += '</li>';
                            rw += '<li ';
							    if (S_nAuswertung == 1) {
							        rw += ' class="wom_clickable" id="link_gw_ergebnis">';
							        rw += '<a href="" onclick="ergebnis(4);return false;">' 
                            ...
                            rw += '<li';
							    if (S_nAuswertung == 1) {
							        ...
							        rw += '<a href="" onclick="ergebnis(5);return false;">' 
                    ...
                    rw += '<header>';
					    ...
					    rw += '<p style="margin-bottom:25px">' + wom_html(WOMT_aTexte["2_text_text"][S_nSprache])+ '<\/p>';
    					if (CONST_VOTEMATCH_OFF != 1) {
                            rw += '<div class="wom_einrueckung" id="votematch_hinweis">';
                                rw += '<span>&nbsp;<\/span>';
                                rw += wom_html(WOMT_aTexte["2_votematch_text"][S_nSprache]);
                            rw += '<\/div>';
                            rw += '<span class="v-hidden wom_einrueckung" id="votety">';
                                rw += '<span>&nbsp;</span>';
                            rw += wom_html(WOMT_aTexte["2_votematch_text_hinweis"][S_nSprache]);
                            rw += '</span>';
                            rw += '<br\/><br\/>';
                        } 

                    rw += '<\/header>';
			        rw += '<span style="float:left;font-size:16px;line-height:20px">' 
    				...
    				rw += '<form enctype="multipart/form-data" aria-describedby="form-desc" id="form_themen" name="themen" method="get" action="main_app.html">\n';
    				    rw += '<input type="hidden" id="neuer_status" name="status" value="';
					        if (S_nAuswertung==1) {
					            rw += '4';
					        } else {
					            rw += '3';
                        ...
                        rw += '<ul class="wom_auswertung" id="bngewichtung" role="tablist">\n';
                    	    ...
                    	    rw += '<li class="wom_default" style="clear:both;"> \n';
                    	        rw += '<div style="width:100% !important;height:32px !important"> \n';
                        	        rw += '<div ';
                        	        if (CONST_VOTEMATCH_OFF != 1) {
                                        if (WOMT_aThesenVotematch[a]== 1) {
                                            rw += ' aria-describedby="markvm'+nr+'"';
        					            }
        					        }
                        	        
                        	        rw += 'class="wom_langethese_tab' + triggerClass2 + '" role="tab" ' + ariaDiv + ' tabindex="-1">\n';
                                    ...
                                    rw += '<div style="overflow: hidden;" class="wom_trigger' + triggerClass + '">\n';
    									rw += '<label ' + ariaLabel + ' id="wom-tabpanel-label-'+ nr + '" class="wom_label_check wom_label_over ' 
                                            ...
                                            rw += '&quot; doppelt gewichten" for="these' + a + '" tabindex="-1">\n';
                                                ...
                                                if (CONST_VOTEMATCH_OFF != 1) {
                        					        if (WOMT_aThesenVotematch[a]== 1) {
                                                        rw += '<span><span id="markvm'+nr+'" class="votematch_description">';
                                                            rw += wom_titletag(WOMT_aTexte["2_votematch_text_markierung_description"][S_nSprache]);
                                                             rw += '</span><img ';
                                                                rw += ' style="font-size:11px;min-width:8px;width:auto;display: inline-block;overflow:fhidden;height:15px !important;"  ';
                                                                rw += ' src="media/pix/icon/stern.png"  ';
                                                                rw += ' width="8"  ';
                                                                rw += ' height="15"  ';
                                                                rw += ' title="' + wom_titletag(WOMT_aTexte["2_votematch_text_markierung_alt"][S_nSprache]) + '" alt="">';
                                                        rw += ' </span> ';
                        					        }
                        					    }
                                        ...
                                        rw += '<\/span>\n';
								    rw += '<\/div>\n';
                                    rw += '<input aria-describedby="doppelty" class="wom_hidecheckbox gwc" type="checkbox" ' 
                                        + checked + 
                                        ' id="these' + a 
                                        + '" name="cb_themen_' 
								...
								rw += '<div style="clear:both" aria-hidden="false" role="tabpanel" id="wom-tabpanel-' 
								        + nr + '" aria-labelledby="wom-tabpanel-label-' 
								        + nr + '"><p>\n';
								    ...
								    switch(S_aThesen[a]){
								       case -2:
								            ...
								            imgAntwort          = '<img src="media/pix/icon/votum_enthalten.png" alt="' 
								                + wom_titletag(ichAntwortText.replace('[THESE]',WOMT_aThesen[a][S_nSprache][1]))
								                + '" />';
								            break;
								        case -1:
        				...
        				rw += '<div class="wom_skip" id="bnskip">';
                            rw += '<a class="wom_previous" href="" onclick="change_frage('
                							...
                							+ '<\/a>';
        					rw += '<input class="wom_next" type="submit" onclick="';
        					    if (S_nAuswertung == 1) {
        					        rw += 'ergebnis(4)';
        					    } else {
        					        rw += 'ergebnis(3)';
        					    }
        					rw += ';return false;"  value="' 
        					    + wom_titletag(WOMT_aTexte["2_form_submit_text"][S_nSprache]) + '" title="' 
        					    + wom_titletag(WOMT_aTexte["2_form_submit_title"][S_nSprache]) + '" />';
        				rw += '<\/div>';
                        rw += '<span class="v-hidden" id="doppelty">'
                            + wom_html(WOMT_aTexte["2_bn_hinweis_gewichtung"][S_nSprache]) 
                            + '</span>';
        			rw += '<\/form>';
	            rw += '<\/main>';
            rw += '<\/div>';
