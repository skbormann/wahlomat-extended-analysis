...
function write_0_intro(){
    ...
    rw += '<\/div>';
    rw += '<div class="wom_lang">';
        rw += '<a href="" onclick="change_language(0);return false;" ';
        if (S_nSprache == 0) {
            rw += 'class="on" ';
        } else {
            rw += 'class="" ';
        ...
        rw += '>'
            + wom_html(WOMT_aTexte["0_sprache_deutsch"][S_nSprache])
            + '<\/a> |';
        rw += '<a href="" onclick="change_language(2);return false;" ';
        if (S_nSprache == 2) {
            rw += 'class="on" ';
        } else {
            rw += 'class="" ';
        ...
        rw += '>'
            + wom_html(WOMT_aTexte["0_sprache_friesisch"][S_nSprache]) 
            + '<\/a> |';
        rw += '<a href="" onclick="change_language(1);return false;" ';
        if (S_nSprache == 1) {
            rw += 'class="on" ';
        } else {
            rw += 'class="" ';
        ...
        rw += '>'
            + wom_html(WOMT_aTexte["0_sprache_daenisch"][S_nSprache]) 
            + '<\/a>';
    rw += '<\/div>';
    
    rw += '<a class="wom_begin" href="" onclick="replaceIFrame(1);return false;" tabindex="20" tabindex="20" title="' 
