...
function check_umfrage() {
    ...
    if (Math.floor($.now()/1000) > 1567360500 ) {     // 1.9.2019 17:55:00
        return false;
    } 
    ...
    if (Math.floor((Math.random() * umfrage_max)) <= umfrage_often) {
        umfrageurl              = 'https://umfrage.wahl-o-mat.de/brandenburg2019/umfrage.html';
        waitmilliseconds        = 15000;
        S_UmfrageJUrl           = 'https://umfrage.wahl-o-mat.de/brandenburg2019/umfrage.html';                  
...
function umfrage_show_div() {
    ...
    htmlUmfrage += '<div>';
        ...
        htmlUmfrage +=  '">' + wom_html(WOMT_aTexte["4_umfrage_close"][S_nSprache]) + '<\/button>';
    htmlUmfrage +=  '<\/div>';
...
function umfrage_open() {
 	umfrage_win = window.open(S_UmfrageOpenUrl, 'UmfrageBrandenburg2019', S_UmfrageJPopupOptions);
