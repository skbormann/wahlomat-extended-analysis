...
function parteiergebnis(zumErgebnis) {
    ct = WOMT_nParteien;
    
    bAll = true;
    for (a = 0; a < ct; a++) {
        val = 'cb_parteien_' + a;
        if (document.forms['formparteiauswahl'].elements[val].checked == true) {
    ...
    if(bAll == true) {
        S_nFilter_ParteienFilter  = 0;
        S_nFilter_ParteienAuswahl = 2;
    } else {
        S_nFilter_ParteienFilter  = 1;
        S_nFilter_ParteienAuswahl = 1;
    }
    
    setDetailergebnisPartei(-1);
    
    if (GetParteienAusgewaehlt()<1){
        // Auf sich selbst setzen!
        S_nParteiauswahlFirst   = 0;
        S_nAuswertung           = 0;    
        replaceIFrame(3);
        ...
        if ((zumErgebnis == 5) && (S_nAuswertung == 1)) {
            replaceIFrame(5);
        } else if (zumErgebnis == 2) {
            replaceIFrame(2);
        } else {
            replaceIFrame(4);
        }
        ...
        rw += '<div class="wom_wrapper">';
            rw += '<div class="wom_content" id="content">';
                ...
                rw += '<main class="wom_main_content">';
                    ...
                    rw += '<nav class="wom_breadcrumb" id="bnbreadcrumb">';
                        ...
                        rw += '<ul class="wom_breadcrumb_list">';
							rw += '<li class="wom_first_position wom_clickable" onclick="replaceIFrame(2);return false;">';
							    rw += '<a href="" onclick="parteiergebnis(2);return false;">' 
							        + wom_html(WOMT_aTexte["bc_2_gewichtung"][S_nSprache])
							        + '<\/a>';
                            rw += '</li>';
							rw += '<li class="wom_active" aria-current="page" title="' 
							    + wom_titletag(WOMT_aTexte["bc_hier"][S_nSprache]) 
							    + ' ' 
							    + WOMT_aTexte["bc_3_partei"][S_nSprache]+ '"><strong>' + wom_html(WOMT_aTexte["bc_3_partei"][S_nSprache])+ '</strong></li>';
                            rw += '<li ';
							    if (S_nAuswertung == 1) {
							        rw += ' class="wom_clickable" id="link_gw_ergebnis">';
							        rw += '<a href="" onclick="parteiergebnis(4);return false;">' 
							        + wom_html(WOMT_aTexte["bc_4_ergebnis"][S_nSprache])
							        + '<\/a>';
							    } else {
							        rw += '><span>' + wom_html(WOMT_aTexte["bc_4_ergebnis"][S_nSprache])+ '</span>';
							    }
							    rw += '</li>';
                            rw += '<li';
							    if (S_nAuswertung == 1) {
							        rw += ' class="wom_clickable" id="link_gw_detailergebnis">';
							        rw += '<a href="" onclick="parteiergebnis(5);return false;">' 
							        + wom_html(WOMT_aTexte["bc_5_begruendungen"][S_nSprache])
							        + '<\/a>';
							    } else {
							        rw += '><span>' + wom_html(WOMT_aTexte["bc_5_begruendungen"][S_nSprache])+ '</span>';
                    ...
                    rw += '<form class="wom_parteien" enctype="multipart/form-data" name="formparteiauswahl" id="formparteiauswahl" method="get" action="main_app.html">\n';                        
                        rw += '<p style="margin-bottom: 0;">';
                        ...
                        lTextDisplay = 'block';
                        if (S_nParteiauswahlFirst == 1) {
                            lTextDisplay = 'none';
                        }
                        rw += '<p aria-live="polite" class="wom_partei_meldung"><span id="fehlertext" style="display:'
                            + lTextDisplay
                            + ';">';
                            if (lText !== false) {
                                rw += lText;
                        ...
                        rw += '<fieldset id="bnparteiauswahl" aria-describedby="legend-desc">';    
                                                ...
                                                rw += '<div class="wom_partei_openclose" id="wom-tabpanel-' + lFixKey + '-oc" role="tab" aria-controls="wom-tabpanel-' + lFixKey + '">';
                                                    rw += '<span class="wom_partei_open"><span class="v-hidden">';
                                                        ...
                                                        rw += '<\/span><img src="./media/pix/icon/arrow_top_large.svg" alt="';
                                                            ...
                                                            rw += wom_titletag(text2.replace('[PARTEI]',WOMT_aParteien[lFixKey][S_nSprache][1]));
                                                        rw += '" /><\/span><\/div>';
                                                rw += '<input style="display:block;visibility: visible;" type="checkbox" onchange="count_check_box(1)"'
                                                    + ' onclick="count_check_box(1)" id="cb_parteien_' 
                        ...
                        rw += '<\/fieldset>\n';
                        rw += '<div class="wom_clear"><\/div>'; 
                        rw += '<fieldset aria-describedby="legend-desc">'; 
                            rw += '<legend>'
    					    ...
    					    rw += '<div class="wom_clear"><\/div>';
				        rw += '<\/fieldset>';
				        rw += '<fieldset id="bnalleparteien">';
                            rw += '<legend>';
            	    		    rw += wom_html(WOMT_aTexte["3_text_unten_auswahl_legend"][S_nSprache]);
                            rw += '</legend>';
                            rw += '<div class="wom_all_checkbox">';
                            rw += '<input style="display:block;visibility: visible;" type="checkbox" id="cb_all_parteien" name="cpv_all" value="1" tabindex="0" class="wom_hidecheckbox2 mark-all-checkbox"';
                                allParty = true;
                                for (a = 0;a<WOMT_nParteien;a++){
                            ...
                            rw += '/>';
                            rw += '<label class="wom_all_label" for="cb_all_parteien">';
                                rw += wom_html(WOMT_aTexte["3_checkbox_alleparteien_text"][S_nSprache]);                            
                                //<span class="sr-only">';
    	                            //rw += wom_html(WOMT_aTexte["3_checkbox_alleparteien_mit"][S_nSprache]);
    	                        //rw += '</span> ';
    	                            //rw += wom_html(WOMT_aTexte["3_checkbox_alleparteien"][S_nSprache]);
    	                        //rw += '<span class="sr-only"> ';
    	                            //rw += wom_html(WOMT_aTexte["3_checkbox_alleparteien_vergleichen"][S_nSprache]);
    	                        //rw += '</span>.
    	                        rw += '<span class="bg">&nbsp;</span></label>';
                            rw += '</div>';
                        rw += '</fieldset>';
			    		rw += '<div class="wom_clear"><\/div>';
    					...
    					rw += '<div class="wom_skip" id="bnskip">';
        					rw += '<a href="" onclick="replaceIFrame(\'2\');return false;" class="wom_previous" title="' 
                                ...
                                + wom_html(WOMT_aTexte["3_btn_zurueck_text"][S_nSprache])
                                + '<\/a>';
                            rw += '<input type="submit" class="wom_next" onclick="parteiergebnis(4);return false;"  value="' 
...
function count_check_box(resetCheckAll){
    lCount = 0;
    for(a = 0;a<WOMT_nParteien;a++){
    ...
    if (resetCheckAll) {
        // Die Hidden cpv_all Checkbox setzen / resetten auch wenn man manuell die 
        // Checkboxen gesetzt hat
        if (lCount < WOMT_nParteien) {
            $('#cb_all_parteien').prop('checked',false); 
        } else {
            $('#cb_all_parteien').prop('checked',true); 
        }
    }    
...
function get_text_from(pSelected,pMin,pMax){
    ...
    if (pSelected<pMin){
        lNewText = text_partei_zuwenige+"<span class='counter'>&nbsp;<\/span>";
        return lNewText;
    }
    return false;
...
function put_text(lFehlerText){
    fehlerDiv = $('#fehlertext');
    if (fehlerDiv){
        if (lFehlerText===false) {
            fehlerDiv.css('display','none');
            fehlerDiv.html('');
        } else {
            fehlerDiv.css('display','block');
            fehlerDiv.html(lFehlerText);
        }
    }
