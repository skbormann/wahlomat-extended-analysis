					...
					if((S_nWeissNicht 	>= CONST_WOMT_NOERGEBNIS_THESEUEBERSPRINGEN)
				     ...
				     ||(S_nSame    	>= CONST_WOMT_NOERGEBNIS_SAME)
				     	...
				     	if (CONST_WOMT_NOERGEBNIS_BEGRUENDUNGEN == 1) {
        					...
        					rw += '<a href="" onclick="replaceIFrame(5);return false;"><span>' 
						        +  wom_html(WOMT_aTexte["4_kommentar_jnv"][S_nSprache]) 
						        + '</span>';
						    rw += '<br /><span>' 
                        ...
                        for (f=0;f<WOMT_nParteien;f++) {
							...
							if (S_aParteienAusgewaehlt[a]==1) {
								...
								rw += 'ergebnis_balken"><span class="ergebnis_partei">' 
								    ...
								    if ((CONST_WOMT_CDUCSU == 1) && (a == CONST_WOMT_CDUCSU_NR)) {
								         rw += '<br/><a href="' 
                                        ...
                                        lText = lText.replace('\[PARTEI\]','CDU');
                                         
                                        rw += wom_titletag(lText); 
                                        rw += '" class="more_info_cc">' 
                                            +  wom_html(WOMT_aTexte["3_mehr_informationen_text_cducsu"][S_nSprache]) 
                                            + ' CDU</a>';
								        rw += '<br/><a href="' 
								            +  CONST_WSZW_URL 
								            + '?partei=' + WOMT_aParteien_N2ID[a] 
								            + '&amp;s=2' + '" target="_blank" title="'; 
                                        lText = WOMT_aTexte["3_mehr_informationen_title"][S_nSprache];
                                            lText = lText.replace('\[PARTEI\]','CSU');
                                        ...
                                        rw += '</p><span class="close_two">&nbsp;</span></div></div></li>';
